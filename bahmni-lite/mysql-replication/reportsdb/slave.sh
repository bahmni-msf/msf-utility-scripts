...
until docker exec bahmni-lite-reportsdb-1 sh -c "$mysql_root_cmd"
do
...
slave_status=$(docker exec bahmni-lite-reportsdb-1 sh -c "$check_slave_status_cmd")
...
docker exec bahmni-lite-reportsdb-1 sh -c "$change_master_cmd"

docker exec bahmni-lite-reportsdb-1 sh -c "export MYSQL_PWD=\"$MYSQL_ROOT_PASSWORD\"; mysql -u $MYSQL_ROOT_USER -e 'SHOW SLAVE STATUS \G'"
